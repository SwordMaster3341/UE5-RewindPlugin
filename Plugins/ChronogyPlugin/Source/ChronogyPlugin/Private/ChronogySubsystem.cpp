...
{
	CurrentTimeDilation = GetWorld()->GetWorldSettings()->SetTimeDilation(Dilation);
}
...
void UChronogySubsystem::RegisterRewindableFX(UNiagaraComponent* FXComponent, EChronogyParticleRewindMode Mode)
{
	...
	Track.DeathAge = -1.f;
	Track.bOwnerAutoActivate = FXComponent->bAutoActivate;
...
void UChronogySubsystem::ConfigureTrackForRewind(FChronogyParticleTrack& Track, float SeekDelta)
{
	...
	if (Track.Mode != EChronogyParticleRewindMode::Scrub || Track.bScrubReady)
		return;
...
{
	UNiagaraComponent* C = Track.Component.Get();
	if (!C || Track.BirthTime < 0.f)
		return; // never born — nothing to reverse
	...
	switch (Track.Mode)
	{
		case EChronogyParticleRewindMode::Scrub:
			if (!C->IsActive())
				C->Activate(false);
			C->SetDesiredAge(FMath::Min(Age, Track.MaxScrubAge));
			break;
...
void UChronogySubsystem::RestoreTrack(FChronogyParticleTrack& Track, float StopClock, float Now)
{
	...
	C->bAutoActivate = Track.bOwnerAutoActivate;
	...
	if (Track.BirthTime >= 0.f)
	{
		...
		{
			Track.BirthTime = Now - FMath::Min(AgeAtStop, Track.MaxScrubAge);
			Track.DeathAge = -1.f;
			Track.bWasActive = false;
			return;
		}
		// Stopped after it had already finished: it must be inactive at this point in history.
		if (C->IsActive())
			C->DeactivateImmediate();
	}
